...
def screenshot_loop(interval=10):
    ...
    base_dir = os.path.join(appdata, "ActivityWatch", "data")
    ...
    with mss.mss() as sct:
        while True:
            ...
            for i, monitor in enumerate(sct.monitors[1:], start=1):
                ...
                # Large version
                large_img = im.resize((1920, 1080), Image.LANCZOS)
                large_filename = os.path.join(large_dir, f"{timestamp}_{i}.jpg")
                large_img.save(large_filename, "JPEG", quality=30, optimize=True)
                # Small version
                small_img = im.resize((192, 108), Image.LANCZOS)
                small_filename = os.path.join(small_dir, f"{timestamp}_{i}.jpg")
                small_img.save(small_filename, "JPEG", quality=30, optimize=True)
            time.sleep(interval)
